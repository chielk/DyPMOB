...
\begin{document}
	...
	\label{sec:approach}
		...
		The planning algorithm was implemented in C++.
		The goal of the algorithm is to compute the values for every action in
		every state of every time.
		As the number of states and time steps is known, all states 
		% generic: any number of states and agents
...
\end{comment}
	...
	\begin{figurehere}
		\centering
		\includegraphics[width=\columnwidth]{images/pareto_difference}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\State $Q[][] = Q[\mathit{max\_t}][\mathit{n\_states}]$ \Comment{Initialise Q}
			...
			\State $\mathit{rewards}[] \gets \Call{get\_rewards}{a, s, \mathit{n\_states}, \mathit{p\_msg}}$
			...
			\State $\Call{increment}{}(r[\Call{action2index}{a}])$
			\Comment{Add direct reward}
			\State $Q[t][s].\Call{add}{a, r}$ \Comment{Try to add to set}
			\EndFor
			\EndIf
			\EndFor
			\EndFor
